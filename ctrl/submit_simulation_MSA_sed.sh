#!/bin/bash
#SBATCH -N PROC_COSMO_ALL  --ntasks-per-node=SIM_NTASKS_NODE -p SIM_PARTITION 
#SBATCH hetjob
#SBATCH -N PROC_CLM_ALL --ntasks-per-node=SIM_NTASKS_NODE -p SIM_PARTITION
#SBATCH hetjob
#SBATCH -N PROC_PARFLOW_ALL --ntasks-per-node=GPU_NTASKS_NODE --gres=gpu:GPU_NTASKS_NODE -p GPU_PARTITION 
